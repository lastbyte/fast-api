...
async def create_user_role(db: SessionDep, user_role_request: CreateUserRoleRequest):
    try:
        ...
        return UserRole.query.add(user_role)
    except Exception as e:
        raise Exception(f"Error creating user role: {e}")
    

async def get_user_role(db: SessionDep, user_role_id: int):
    try:
        user_role = UserRole.query.filter(UserRole.id == user_role_id).first()
        return user_role
    except Exception as e:
        raise Exception(f"Error getting user role: {e}")
    

async def update_user_role(db: SessionDep, user_role_id: int, user_role_request: CreateUserRoleRequest):
    try:
        user_role = get_user_role(db=db, user_role_id=user_role_id)
